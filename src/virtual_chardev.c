 ...
 *  Возвращает сообщение с указанием количества произведённых попыток чтения из
 *  файла устройства.
 */
...
#define DEVICE_NAME "chardev" /* Имя устройства, см. /proc/devices */
...
static int device_release(struct inode *inode, struct file *file)
{
    ...
    /*
     *  Уменьшить счетчик обращений, иначе, после первой же удачной попытки
     *  открыть файл устройства, вы никогда не сможете выгрузить модуль.
     */
                           ...
                           loff_t * offset)
{
    ...
    {
      /*
       *  Буфер находится в пространстве пользователя (в сегменте данных), а не
       *  в пространстве ядра, поэтому простое присваивание здесь недопустимо.
       *  Для того, чтобы скопировать данные, мы используем функцию put_user(),
       *  которая перенесёт данные из пространства ядра в пространство
       *  пользователя.
       */
...
/*
 *  Вызывается, когда процесс пытается записать в устройство, например, так:
 *  echo "hi" > /dev/chardev
 */
